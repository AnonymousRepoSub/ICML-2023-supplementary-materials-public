...
def init_electricity(split='train', shuffle=False):
    alldata = arff.load(open('data/electricity.arff', 'r'), encode_nominal=True)
...
def init_sales(split='train', shuffle=False):
    ...
    if split == 'train':
        ...
        #     train_group_value += [index]*num 
        # train_group_value =np.array(train_group_value)

        # train_group_num = 4
        # group_index = list(set(train["month"]))
        # print(group_index, flush=True)
        # train_group_value = []
        # for index in range(0, train_group_num*2, 2):
        #     num = len(train[(train["month"] == group_index[index]) | (train["month"] == group_index[index+1])])
        #     train_group_value += [index/2]*num 
        # train_group_value =np.array(train_group_value)
        train_group_num = 4
        train_group_value = None
...
def init_temp(data_size, split='train', shuffle=False):
    import xarray as xr
    ds = xr.open_dataset("data/temp/003_2006_2080.nc")
...
def init_vessel(data_size, split='train', shuffle=False):
    train = pd.read_csv("data/power_consumption_upload/synthetic_data/train.csv")
    if data_size == 100000:
        train = train[:100356] #  up to time id 518400, around 12 month
    test = pd.read_csv("data/power_consumption_upload/synthetic_data/dev_out.csv")
    if shuffle:
